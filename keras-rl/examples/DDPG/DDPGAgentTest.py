...
class DDPGAgent(Agent):
    ...
    def select_action(self, state):
        #state_left = state
        #state_right = process_state_right(state)

        #batch_left = self.process_state_batch([state_left])
        #batch_right = self.process_state_batch([state_right])

        #action_left = self.actor.predict_on_batch(batch_left).flatten()
        #action_right = self.actor.predict_on_batch(batch_right).flatten()
    ...
    def backward(self, reward, terminal=False):
        ...
        if can_train_either and self.step % self.train_interval == 0:
            ...
            if self.step > self.nb_steps_warmup_actor:
                ...
                action_values = self.actor_train_fn(state_inputs)[0]
